...
class YoloEObjectDetectionOperator(BaseOperator):
    """Runs YOLOE inference to auto-label images with bounding box annotations."""

    name: str = "YOLOE Open Vocabulary Object Detection"
    description: str = (
        "Runs YOLOE inference and adds bounding box annotations to unlabeled images."
    )
    ...
    def parameters(self) -> list[BaseParameter]:
        ...
        return [
            StringParameter(
                ...
                description="YOLO model weights path or Ultralytics model name (e.g. yoloe-26n-seg.pt).",
            ),
            FloatParameter(
            ...
            StringParameter(
                name=PARAM_CLASS_NAMES,
                required=True,
                description="Comma-separated list of class names for object detection.",
            ),
            StringParameter(
    ...
    def execute(
        ...
        if not parameters.get(PARAM_CLASS_NAMES):
            return OperatorResult(
                success=False,
                message="class_names parameter is required and cannot be empty.",
            )
        class_names = [name.strip() for name in parameters[PARAM_CLASS_NAMES].split(",") if name.strip()]
        if not class_names:
            return OperatorResult(
                success=False,
                message="class_names parameter must contain at least one valid class name.",
            )
        ...
        # Get all images in the current collection with the given context filter
        ...
        return OperatorResult(
            success=True,
            message="YOLOE Object Detection executed successfully.",
        )
